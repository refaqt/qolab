...
3. **Mint with `p = c0 · c^k`.** Do not quote `dc/dp` in euros. Sub-projects use nested mint budgets ([nested-mint-budgets.md](nested-mint-budgets.md)): local `p_j` does not move the global curve until the parent admits points.
