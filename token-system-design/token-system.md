...
| How else to decide how many certificates to burn per euro?                                                                                                                 | Do **not** derive burn from a bonding-curve euro rate, and **do not** use `v = pool / outstanding` or `v = pool / N_cap` either. Those are still a spot price on a stock: holders wait, then run; the pot sits idle; same-year deduction never happens. There is no “euros per certificate” to compute. Each year split a **vintage budget** `P_t` across weights (default-in); do not burn for taking the vintage; leftover to reserves. Early-bird stays in the **mint** `p = c0 · c^k`. Details: [vintage-pools.md](vintage-pools.md). |
