...
8. **DVB + counsel** before anyone is promised a certificate. Ask explicitly: (a) no taxable event at mint; (b) characterisation of cash-out as fee vs dividend; (c) VAT time of supply; (d) RSZ / arbeidsrelatie; **(i)** an effort-tied fee cap vs implied `€/p` at grant (`(k × E) / Δp`); **(j)** proportional `p` burn against remaining opened cap vs remaining consideration — this binds the next option-2 pass, not only parked vintages. Nested mint ([nested-mint-budgets.md](nested-mint-budgets.md)): **(k)** delayed root-`p` mint after unofficial work; **(l)** `p_j` as a non-claim on `P_t` (split key for the subpool); **(m)** parent-chosen mint budgets vs implied project value; **(n)** fee-cap clock at first root `p`; **(o)** project entity as invoicing contractor; contributors invoice the subpool. Vintage-specific items (e)–(h) stay on the parked note; they are not live until that packaging returns.
