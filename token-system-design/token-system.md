...
8. **DVB + counsel** before anyone is promised a certificate. Ask explicitly: (a) no taxable event at mint; (b) characterisation of cash-out as fee vs dividend; (c) VAT time of supply; (d) RSZ / arbeidsrelatie; **(i)** an effort-tied fee cap vs implied `€/p` at grant (`(k × E) / Δp`); **(j)** proportional `p` burn against remaining opened cap vs remaining consideration — this binds the next option-2 pass, not only parked vintages. Vintage-specific items (e)–(h) stay on the parked note; they are not live until that packaging returns.
