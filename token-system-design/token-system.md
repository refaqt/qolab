...
4. **No unit `v`.** Do not burn using `pool / outstanding` or `pool / N_cap`. Do not burn `p` at a euro par. Vintages (default-in split of this year’s `P_t`, leftover to reserves, no burn for taking the flow, time-opened cap) are **parked for complexity**, not because that analysis was wrong ([vintage-pools.md](vintage-pools.md)). Cheap extra shares whose number tracks `p` are a **dead-end** ([cheap-share-route.md](cheap-share-route.md), [ADR 2026-09-04](../docs/decisions/2026-09-04_cheap-share-route-dead-end.md)). Next pass is **option 2** with less machinery.
...
8. **DVB + counsel** before anyone is promised a certificate. Ask explicitly: (a) no taxable event at mint; (b) characterisation of cash-out as fee vs dividend; (c) VAT time of supply; (d) RSZ / arbeidsrelatie; **(i)** an effort-tied fee cap vs implied `€/p` at grant (`(k × E) / Δp`) — this binds the next option-2 pass, not only parked vintages. Vintage-specific items (e)–(h) stay on the parked note; they are not live until that packaging returns.
