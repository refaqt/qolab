...
**Replacement (parked packaging):** [vintage-pools.md](vintage-pools.md). Each year the GA/board sets a budget `P_t`. Split it across eligible weights with **default-in**. Leftover goes to **operating reserves**, not back into a certificate pool. **Do** amortize `p` against remaining opened cap ([p-burn-remaining-cap.md](p-burn-remaining-cap.md)) — that is not a unit FX. That idle-pool analysis still stands; the lot / time-opened-cap packaging is **parked for complexity**.

**Not a replacement:** burning `p` at a euro par via cheap extra shares ([cheap-share-route.md](cheap-share-route.md)) — **dead-end**. Next pass is option 2 with less machinery, still without a unit `v`.

A fee cap on **documented effort** is not a substitute unit price. `(k × hours × rate) / Δp` is a personal rewrite of that ceiling; it is not a conversion rate the rules use, and it is not the same for every outstanding `p`. Do not publish it. Full argument: [vintage-pools.md](vintage-pools.md).
...
3. **Mint with `p = c0 · c^k`.** Do not quote `dc/dp` in euros. Sub-projects use nested mint budgets ([nested-mint-budgets.md](nested-mint-budgets.md)): local `p_j` does not move the global curve until the parent admits points; root `p` is credited to the **project entity**, which invoices `P_t` into a euro subpool ([ADR 2026-09-09](../docs/decisions/2026-09-09_project-as-contractor.md)).
4. **No unit `v`.** Do not burn using `pool / outstanding` or `pool / N_cap`. Do not burn `p` at a euro par. **Do** amortize `p` by `alloc / rem` of remaining opened cap ([p-burn-remaining-cap.md](p-burn-remaining-cap.md)). Vintages (default-in split of this year’s `P_t`, leftover to reserves, time-opened cap) are **parked for complexity**, not because that analysis was wrong ([vintage-pools.md](vintage-pools.md)). Cheap extra shares whose number tracks `p` are a **dead-end** ([cheap-share-route.md](cheap-share-route.md), [ADR 2026-09-04](../docs/decisions/2026-09-04_cheap-share-route-dead-end.md)). Next pass is **option 2** with less machinery.
