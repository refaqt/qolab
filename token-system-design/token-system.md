...
| Does a fee cap `k × hours × rate` put a price on `p` the same way? (`(k × E) / Δp`)                                                                                         | **No as a unit FX.** That cap is on the **invoice versus documented effort**, not on minted `p`. Same work can mint different `Δp`, so the quotient is not a property of `p`. A ceiling is not consideration “to be received” (WBTW art. 26). What *does* price the mint is `w`, `v = P/N`, or remaining `p` = remaining euros. Do not publish `(k × E) / Δp` or book the cap as a receivable. Full argument: [vintage-pools.md](vintage-pools.md) (“Does `(k × hours × rate) / Δp` price `p`?”). Still a DVB hypothesis (item **(i)**). |
...
A fee cap on **documented effort** is not a substitute unit price. `(k × hours × rate) / Δp` is a personal rewrite of that ceiling; it is not an FX the rules use, and it is not the same for every outstanding `p`. Do not publish it. Full argument: [vintage-pools.md](vintage-pools.md).
