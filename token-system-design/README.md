...
| **Capped participating bond + two-leg waterfall** | **Open (proposed)** ([ADR 2026-09-14 investor leg](../docs/decisions/2026-09-14_investor-leg-capped-participating-bond.md); [ADR 2026-09-14 hardware](../docs/decisions/2026-09-14_hardware-via-bond-and-loan.md)) | The first path that pays **capital**: a subordinated, contingent-repayment bond whose row accrues `r_I` on outstanding principal, **never freezes**, and is lidded at `k_I · I`; `P_t` is split between contributors and holders by `θ = Σ rem^C / (Σ rem^C + Σ rem^I)`, by formula and not by vote; uniform terms, majority amendment, no individual veto. **Hardware for experiments:** the contributor subscribes to this same bond; the CV buys the tool and lends it | [paths/capped-participating-bond/](paths/capped-participating-bond/README.md) |
| **Governance leg — √-count vote-only shares, veto share, asset lock** | **Open (proposed)** ([ADR 2026-09-15](../docs/decisions/2026-09-15_votes-off-the-reward.md)) | The first path that pays nobody: one class of vote-only shares at €1, returned at €1; a member may hold `N_i = max(1, ⌊√(V_i/1000)⌋)` shares where `V` is decayed accepted contribution (`δ = 0.25`); one share one vote, no spending; vote cap `max(10%, k/N)`; investors stay on the bond with no vote; a class G veto share held by a foundation; an asset lock and a mission lock in the statutes | [paths/governance-voting/](paths/governance-voting/README.md) |
...
- Governance membership is **not** the reward. Share count never tracks contribution **on a share with a profit right**. The governance share has no profit right; its count follows the square root of decayed contribution and nothing is ever minted from `p` ([governance leg](paths/governance-voting/README.md)). Votes never sit on the fee or on the bond. No document states euros per hour or per `c`.
...
- Do not publish 10×, `r`, a target ROI, or “beats ETFs” language anywhere. **Two** carve-outs. Since [ADR 2026-09-14 one-row cap](../docs/decisions/2026-09-14_simplified-cap-one-row.md): a **contracted contributor** may receive a statement of **their own** ceiling, under the limits in [annex-terms.md](paths/option-2-contingent-fee/annex-terms.md) clauses 24–27. Since [ADR 2026-09-14 investor leg](../docs/decisions/2026-09-14_investor-leg-capped-participating-bond.md): an **investor offering document** may state `r_I`, `k_I` and the cap, because disclosure is compulsory and nobody can be asked for money without the return terms — and it may state **nothing** about the contributor leg. The two instruments never appear in one document ([2026-09-14 mistake](../docs/mistakes/2026-09-14_second-instrument-publishes-the-comparable.md)). Public and promotional material, and any return or yield framing outside a compelled disclosure, stay banned.
- What the website may say before counsel drafts (contributor page; no numbers; capital only on a separate page that is not an offer): [publication-surfaces.md](paths/option-2-contingent-fee/publication-surfaces.md).
...
- Before adding any **second instrument**, check it against the first as a **pair**: list the facts the first instrument's defence relies on being *absent* (no principal, no euro price per hour, no unit conversion rate) and confirm the second does not supply one — including in documents the company is **compelled** to publish ([2026-09-14 mistake](../docs/mistakes/2026-09-14_second-instrument-publishes-the-comparable.md)).
...
| Values of `r_I` and `k_I`; whether the comparable exercise puts them at the same numbers as `r` and `k` or apart | [capped-participating-bond § Open questions](paths/capped-participating-bond/README.md); DVB **(u)** |
| Is a real 10× wanted for **capital**, and therefore two bases for capital as well as for labour? | [capped-participating-bond §6](paths/capped-participating-bond/README.md); [cash-shares path](paths/cash-shares-residual/README.md) |
| Holders'-meeting machinery for a CV bond (art. 6:19): majorities, reserved matters, per-head voting | [capped-participating-bond §5.2](paths/capped-participating-bond/README.md); [loan-regulations.md](paths/capped-participating-bond/loan-regulations.md) clause 16 |
| Art. 344 §1 exposure of the **coupled** waterfall, not only of two instruments side by side | DVB **(t)**; [capped-participating-bond §4.2](paths/capped-participating-bond/README.md) |
| Statutory clauses of the governance leg: entitlement and issuance, automatic partial withdrawal at the cut-off and the six-month window, vote cap, leonine minimum on a vote-only share, class G veto share and reserved matters, asset lock and liquidation destination | [governance-voting §8](paths/governance-voting/README.md); [dvb-questions § Counsel items](analysis/dvb-questions.md#counsel-items-not-numbered-dvb-items) |
| Who holds the class G share before a foundation exists; start values of `δ` and `k` | [governance-voting §8](paths/governance-voting/README.md) |
| A governance-distribution rule that does not rest on hours (self-election vs exclusion) | [governance-voting §4.5](paths/governance-voting/README.md); [docs/governance.md](../docs/governance.md) |
| Hardware for an experiment: benefit in kind, VAT on a loan for use, hire vs free loan, consumable materials | [capped-participating-bond §8.1](paths/capped-participating-bond/README.md#81-hardware-for-experiments); [ADR 2026-09-14 hardware](../docs/decisions/2026-09-14_hardware-via-bond-and-loan.md) |
