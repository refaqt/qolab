...
| **Capped participating bond + two-leg waterfall** | **Open (proposed)** ([ADR 2026-09-14 investor leg](../docs/decisions/2026-09-14_investor-leg-capped-participating-bond.md)) | The first path that pays **capital**: a subordinated, contingent-repayment bond whose row accrues `r_I` on outstanding principal, **never freezes**, and is lidded at `k_I · I`; `P_t` is split between contributors and holders by `θ = Σ rem^C / (Σ rem^C + Σ rem^I)`, by formula and not by vote; uniform terms, majority amendment, no individual veto | [paths/capped-participating-bond/](paths/capped-participating-bond/README.md) |
