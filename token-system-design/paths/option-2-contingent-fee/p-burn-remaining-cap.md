...
5. **One row, with a lid.** Since [ADR 2026-09-14 one-row cap](../../../docs/decisions/2026-09-14_simplified-cap-one-row.md) option 2 keeps one `p_i` and one `rem_i`; continuous application (guard 2) is what fixes the “caps filled, then everyone contributes again” case. What replaces the per-lot protection against a new lot inheriting old waiting is the **lid** `cap ≤ k · E` plus the one-vintage gate on new effort ([spec §5](README.md)). The lid is not optional for this guard: see the paragraph below. The parked vintage note still uses the per-lot fraction `p_L · alloc_L / rem_L` if lots ever return.

Opening rem with `r` while unpaid: `p` stays, `rem` grows, so `p / rem` falls; a later payout then burns fewer tokens per euro. At exhaustion, remaining `p` still goes to 0. **The lid is what keeps that true.** If `rem` could grow without bound — for example under a ceiling written as `cap × (1 + r)` — the burned fraction per euro would fall towards zero, weight would never retire, and the longest waiter would end up holding both the largest ceiling and the most durable weight in every later split. That is the seniority hole this note exists to close, reopened from the other side ([mistake 2026-09-14](../../../docs/mistakes/2026-09-14_cap-as-balance-times-one-plus-r.md)). Do not describe `r` as interest.
