...
| ~~Whether to seek *erkend als sociale onderneming* (art. 8:5 ~6% cap)~~ — **not sought** since [ADR 2026-09-15](../../docs/decisions/2026-09-15_votes-off-the-reward.md); the asset lock is statutory. NRC recognition is a separate, still undecided matter | Closed | [mission.md](../../docs/mission.md) |
...
| ~~The article and permitted deviation for one member, one vote~~ — superseded: the GA votes **one share, one vote** on √-count governance shares, the WVV default, with a per-member cap ([ADR 2026-09-15](../../docs/decisions/2026-09-15_votes-off-the-reward.md)) | Closed | [governance-voting §1.4](../paths/governance-voting/README.md) |
| Governance leg: the **entitlement clause** (`N_i = max(1, ⌊√(V_i/1000)⌋)`) and which body issues shares within it without a statute change | Yearly subscription must be cheap | [governance-voting §4.3](../paths/governance-voting/README.md) |
| Governance leg: **automatic partial withdrawal** of shares above the entitlement at the cut-off (art. 6:120 WVV), the six-month withdrawal window, and the distribution tests on paying €1 per share | The only way share counts go down | [governance-voting §4.3](../paths/governance-voting/README.md) |
| Governance leg: the **vote-cap clause** `max(10%, k/N)` of votes present | Bounds veteran dominance; standard cooperative clause | [governance-voting §1.4](../paths/governance-voting/README.md) |
| Governance leg: **leonine minimum** on a vote-only share — is €1 back on withdrawal and at liquidation enough participation (art. 4:2 WVV)? | A share with neither profit nor liquidation right is void | [governance-voting §4.2](../paths/governance-voting/README.md) |
| Governance leg: the **class G veto share** held by a *stichting* or VZW, and the reserved-matters approval clause | Entrenchment of the mission and the locks | [governance-voting §5.1](../paths/governance-voting/README.md) |
| Governance leg: **asset-lock and liquidation-destination clauses** without *sociale onderneming* recognition; the mission lock incl. CERN-OHL | Non-tax rationale for **(q)** and **(t)** that holds if the tax outcome reversed | [governance-voting §5.2](../paths/governance-voting/README.md) |
| Governance leg: a **class ceiling** on votes (≤ 5% of votes present) if a financial share class is ever created; the WVV departure from one-share-one-vote for that class | Only if the cash-shares path is used | [governance-voting §4.1](../paths/governance-voting/README.md) |
