...
4. Each year the GA/board sets a **vintage budget** `P_t` (waterfall + solvency). Lots minted in year `Y` first sit in vintage `Y+1`. Default-in split across eligible weights, each lot capped by its **time-opened** fee cap (1× at first eligibility; opens only while unpaid). Leftover to reserves; weights are not burned for taking the vintage. No `v = pool / N`.
