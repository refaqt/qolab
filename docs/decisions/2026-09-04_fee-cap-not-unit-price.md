...
- **Date:** 2026-09-04
- **Status:** Proposed. **Amended by** [2026-09-08 proportional `p` burn](2026-09-08_p-burn-remaining-cap.md) (fraction of remaining opened cap is not a unit FX).
...
3. What *does* price the mint, and stays rejected: a published €/unit (`w`, `v = P/N`), remaining `p` as remaining euros (1 `p` = €1), or choosing a denominator so holders hit a target on points. Amortizing `p` by the **fraction** of remaining opened cap just allocated is **not** that unit FX — [ADR 2026-09-08](2026-09-08_p-burn-remaining-cap.md). Do not publish `rem / p`.
4. Ask DVB item **(i)** explicitly: effort-tied fee cap versus implied `€/p` at grant. Ask **(j):** proportional burn versus remaining `p` as remaining consideration.
