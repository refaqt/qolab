...
**Later notes:** see the [first entry](2026-09-12_second-mint-variable-candidates.md) — the 09-14 one-row cap, the 10× moving off the effort base, and votes following decayed accepted contribution all post-date this pass and are not merged into it.
...
- The DVB question stands (no letter claimed — see the first entry), with 16 as the worked example — “a counterparty signed” is the closest thing in the set to pay-for-performance optics, and pay-for-performance is a wage story under **(c)**.
