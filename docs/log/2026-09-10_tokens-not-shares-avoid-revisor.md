...
- Cheap or tenth-euro shares with a `p` quota, with or without burn. Same if there are no investors yet, and same if the quota sits only in a side ledger “until we put it on paper” — [2026-09-10](2026-09-10_no-investors-euro-shares.md). Same if an MoU promises unpaid / “given” / revalued class L based on contribution — [startup-equity MoU](2026-09-10_startup-equity-mou.md).
