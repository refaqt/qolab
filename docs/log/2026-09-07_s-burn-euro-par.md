...
What would **not** price `S`: keep it as a dimensionless weight; split `P_t` by share of `S`; amortize that weight by the fraction of remaining opened cap just allocated (`p · alloc / rem`), not by subtracting euros ([p-burn-remaining-cap.md](../../token-system-design/paths/option-2-contingent-fee/p-burn-remaining-cap.md)). An effort cap on the **invoice** is not a unit FX of `S` (it does not convert tokens into euros).
