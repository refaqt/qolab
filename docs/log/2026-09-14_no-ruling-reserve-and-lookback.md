...
**Status note (2026-09-14, later the same day):** the disallowance allocation this log asks for is now written — the **pool absorbs** it ([annex-terms §C](../../token-system-design/paths/option-2-contingent-fee/annex-terms.md), [ADR 2026-09-14 defaults](../decisions/2026-09-14_exit-death-and-disallowance-defaults.md), Proposed). The “keep time-based work near 1×, put 10× on a scoped royalty or cash shares” next step is [ADR 2026-09-14 two bases](../decisions/2026-09-14_two-bases-10x-off-effort.md) (Proposed). Path map: [token-system-design/README.md](../../token-system-design/README.md).
...
So skipping the DVB mainly leaves the **expensive** questions open: mint vs invoice **(a)**, fee vs dividend **(b)**, VAT time of supply **(c)**, contractor vs employee **(d)**. The 10× number was never going to be locked anyway. That matches [dvb-questions.md](../../token-system-design/analysis/dvb-questions.md) “How to file”: the gate is **(a)–(d)**, not **(p)**.
