...
**Status:** **open** exploration — filed as a log entry only. Nothing here is wired into the [design folder](../../token-system-design/README.md): no path row, no DVB item, no ADR.

**Later notes:** written 2026-09-12, before the decisions of 09-14/09-15. Three of them bear on it and are **not** merged into the text below. (1) The cap became **one row**, so the per-lot framing used here is stale — [ADR 2026-09-14](../decisions/2026-09-14_simplified-cap-one-row.md). (2) The large multiple moved **off the effort base** and option 2 now targets ~1× — [ADR 2026-09-14](../decisions/2026-09-14_two-bases-10x-off-effort.md), [mistake 2026-09-14](../mistakes/2026-09-14_10x-on-the-effort-base.md); a multiplier that sits on the points ledger must not become a back door to that multiple. (3) Votes now follow **decayed accepted contribution** — [ADR 2026-09-15](../decisions/2026-09-15_votes-off-the-reward.md) — so a multiplier on `c` would move governance weight as well as share of `P_t`, an interaction this text predates. The DVB catalog has also moved on: the letter proposed below is taken, and no letter is claimed here.
...
- Draft it into [dvb-questions.md](../../token-system-design/analysis/dvb-questions.md), under whatever letter is free then, only once a multiplier is actually in the spec — the gate before launch is still **(a)–(d)**. Nothing goes into the catalog from this entry.
...
- That same DVB question is unchanged, and now sharper: 16 (“a counterparty signed”) is closer to pay-for-performance optics than anything in the first list. Same question to counsel, with 16 as the worked example.
